...
class _ServiceDetailState extends State<ServiceDetail> {
  ...
  final _serviceDetailCubit = ServiceDetailCubit();
  ...
  void initState() {
    ...
    _serviceDetailCubit.onLoad(widget.id);
  }
  ...
  ///On Share
  /*void _onShare(SingleServiceModel item) {
    Share.share(
  ...
  ///On navigate gallery
   void _onPhotoPreview(SingleServiceModel service) {
    Navigator.pushNamed(
      context,
      Routes.gallery,
      arguments: service,
  ...
  ///Build content UI
  Widget _buildContent(SingleServiceModel? service, StaffModel? staff,
      List<SingleServiceModel>? relatedService) {
    ///Build UI loading
    List<Widget> action = [];
    ...
    if (service != null) {
      ...
      if (_iconColor == Colors.white) {
        backgroundAction =
            UtilColor.getColorFromHex('#1c4c72').withOpacity(0.3);
      }
            ...
            children: <Widget>[
              ...
              ListView.separated(
                ...
                itemBuilder: (context, index) {
                  final item = relatedService[index];
                ...
                itemCount: relatedService.length,
              ),
            ],
          ),
        );
      ...
      ///Phone
      if (staff!.phoneWhatsapp.isNotEmpty) {
        phone = Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const SizedBox(height: 16),
            InkWell(
              onTap: () {
                _phoneAction(staff.phoneWhatsapp);
              },
              child: Row(
                children: <Widget>[
                  Container(
      ...
      if (staff.email.isNotEmpty) {
        email = Column(
          ...
          children: [
            ...
            InkWell(
              onTap: () {
                _makeAction('mailto:${staff.email}');
              },
              child: Row(
                children: <Widget>[
                  Container(
      ...
      ///Tags
      /*if (service.categorie.name.isNotEmpty) {
        tags = Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const SizedBox(height: 8),
            ...
            Visibility(
              ...
              child: Padding(
                ...
                child: Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: product.socials.map((item) {
                    return InkWell(
                      onTap: () {
                        _makeAction(item.value);
                      },
                      child: Container(
                        width: 32,
                        height: 32,
    ...
    return CustomScrollView(
      ...
      slivers: <Widget>[
        SliverAppBar(
          ...
          backgroundColor: UtilColor.getColorFromHex('#1c4c72'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocProvider(
        create: (context) => _serviceDetailCubit,
        child: BlocBuilder<ServiceDetailCubit, ServiceDetailState>(
          builder: (context, state) {
            ...
            if (state is ServiceDetailSuccess) {
              service = state.service.singleServices[0];
              staff = state.service.singleServices[0].staffs[0];
              related = state.service.related;
            }
            return _buildContent(service, staff, related);
          },
        ),
      ),
    );
