...
class ListServices extends StatefulWidget {
  final SingleLocationModel location;

  const ListServices({Key? key, required this.location}) : super(key: key);

  @override
  _ListServicesState createState() {
    return _ListServicesState();
  }
}
...
/*    if (widget.category.type == CategoryType.category) {
  ...
  Future<void> _onRefresh() async {
    await _listCubit.onLoadServicesByLocation(widget.location.id);
  }
  ...
  void _onChangeView() {
    ...
    switch (_listMode) {
      case ServiceViewType.grid:
        _listMode = ServiceViewType.grid;
        break;
      case ServiceViewType.list:
        _listMode = ServiceViewType.block;
        break;
  ...
  void _onIndexChange(SingleServiceModel service) {
    setState(() {
  ...
  void _onLoadDirection() async {
    ...
    if (currentLocation != null && _currentService != null) {
      ...
      if (result.points.isNotEmpty) {
        ...
        AppBloc.messageCubit.onShow('cannot_direction', MessageStatusType.error);
      }
  ...
  ///_build Item
  Widget _buildItem({
    SingleServiceModel? item,
    required ServiceViewType type,
  }) {
    switch (type) {
      case ServiceViewType.list:
        if (item != null) {
    ...
    return BlocBuilder<ListCubit, ListState>(
      builder: (context, state) {
        ...
        if (_pageType == PageType.list) {
          ...
          if (_listMode == ServiceViewType.grid) {
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    Expanded(
                      child: Swiper(
                        itemBuilder: (context, index) {
                          final SingleServiceModel item = state.list[index];
                          bool selected = _currentService == item;
                          if (index == 0 && _currentService == null) {
                            selected = true;
                          }
                          return Container(
